...
const FirstRowSection = ()=>{
    ...
    const {firstRowSectionWidth, setFirstRowSectionWidth, windowWidth, setWindowWidth, categories, openSubCategory, setOpenSubCategory, subCategoryIndex, setSubCategoryIndex} = useGlobalContext()
    ...
    const changeWindowWidth = ()=>{
        setWindowWidth()
        setFirstRowSectionWidth(slideWrapper.current?.clientWidth)
    }   
    useEffect(()=>{
        if(firstRowSectionWidth===0) setFirstRowSectionWidth(slideWrapper.current?.clientWidth)
    ...
    useEffect(()=>{
        ...
        if(slideProductAds==='firstSlide'){
            ...
            slideRef.current.style.transform = position
        }

        else if(slideProductAds==='secondSlide'){
            position = `translateX(-${ windowWidth - (firstSlideSideWidth + (widthOfImagesShowingSideways/2)) }px)`
            slideRef.current.style.transform = position
        }

        else if(slideProductAds==='otherSlides'){
            position = `translateX(-${(prevLeft + (windowWidth - widthOfImagesShowingSideways))}px)`
            slideRef.current.style.transform = position
        }

        else if(slideProductAds==='lastSlide'){
            const productAdsWidth = slideRef.current.clientWidth
            position = `translateX(-${productAdsWidth - window.innerWidth}px)`
            slideRef.current.style.transform = position
        }
        
        
        setCurrentPostionOfSlide(position)
