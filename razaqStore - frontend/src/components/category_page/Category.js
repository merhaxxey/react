...
import MinimumDistanceSlider from '../search/MinimumDistanceSlider'
import SplitButton from '../search/SplitButton'
import {useLocation, useParams} from 'react-router-dom'
import {useSelector, useDispatch} from 'react-redux'
import {setDefaultFilterValues, setIsLoading, setProducts, searchProducts, setFormValue} from '../../features/category/categorySlice'
import Circular from '../progress/Circular'
import Footer from '../homepage/Footer'
import '../search/Search.css'
import '../search/MinimumDistanceSlider.css'
import './Category.css'
import {getCompanies, getMinMaxPrice} from '../search/utils';
...
const Category = ()=>{
    ...
    const {search, formValue, products, defaultFilterValues} = useSelector((store)=> store.category)
    ...
    useEffect(()=>{
        dispatch(searchProducts(Number(categoryId)))
    ...
    return <main id="category-page-id" style={{backgroundColor: 'var(--mainBKColor)'}} className='search-container'>
        <main className={`${windowWidth<=962?'search-wrapper-mobile': 'search-wrapper'}`}>
            ...
            <section className={`product-list-wrapper ${productsToDisplay.length!==2 || 'product-list-wrapper-twoProducts'} ${productsToDisplay.length!==1 || 'product-list-wrapper-oneProducts'}`}>
                ...
                <article className={`${windowWidth<=962? 'product-list-mobile': 'product-list'} `}>
                    ...
                    return <Link to={`/product?name=${name}`} id="exploreStore-product-id" className={`product-list-template exploreStore-product ${windowWidth<=962? 'search-product-mobile': 'search-product'} ${productsToDisplay.length!==2 || 'search-product-twoProducts'} ${productsToDisplay.length!==1 || 'search-product-oneProducts'}`}>
