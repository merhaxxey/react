...
export const getProduct = createAsyncThunk('product/getproduct', async(name)=>{
    try {
        console.log(name)
        const product = await axios.get('/api/v1/product/single?name='+name)
        return product
    } catch (error) {
...
export const {setAddItemsToCart, setIsLoading, setProduct, setWriteReview} = productSlice.actions
