...
const Search = ()=>{
    ...
    const {formValue, products, defaultFilterValues} = useSelector((store)=> store.search)
    ...
    useEffect(()=>{
        dispatch(searchProducts('random'))
    }, [])
    ...
    useEffect(()=>{
        ...
        if(Object.keys(products).lenght !== 0){
        ...
        <main className={`${windowWidth<=962?'search-wrapper-mobile': 'search-wrapper'}`}>
            ...
            <section className={`product-list-wrapper ${productsToDisplay.length!==2 || 'product-list-wrapper-twoProducts'} ${productsToDisplay.length!==1 || 'product-list-wrapper-oneProducts'}`}>
                ...
                <article className={`${windowWidth<=962? 'product-list-mobile': 'product-list'} `}>
                    ...
                    return <Link to={`/product/${name}`} id="moreToLove-product-id" className={`moreToLove-product ${windowWidth<=962? 'search-product-mobile': 'search-product'} ${productsToDisplay.length!==2 || 'search-product-twoProducts'} ${productsToDisplay.length!==1 || 'search-product-oneProducts'}`}>
